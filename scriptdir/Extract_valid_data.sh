...
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$3"\t""+""\t"$6}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.combine.cut.info > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.Combined.linker1.5nd.txt
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$5"\t""+""\t"$8}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.combine.cut.info > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.Combined.linker1.3nd.txt
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$3"\t""+""\t"$6}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.notCombined.1.cut.info  > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.notCombined.linker1.R1.5nd.txt
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$3"\t""+""\t"$6}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.notCombined.1.cut.info  > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.notCombined.linker1.R1.3nd.txt
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$3"\t""+""\t"$6}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.notCombined.2.cut.info  > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.notCombined.linker1.R2.5nd.txt
awk -vT=${Threshold} '{if($2==1 && NF==8 && length($3)>=T && length($5)>=T) print "@"$1"\t"$3"\t""+""\t"$6}' $OUTPUT_DIRECTORY1/$OUTPUT_PREFIX.notCombined.2.cut.info  > $OUTPUT_DIRECTORY2/$OUTPUT_PREFIX.notCombined.linker1.R2.3nd.txt
